...
router.post("/cong-no/upload-misa", upload.single("file"), (req, res) => {
  const store = load();
  ensureMisaShape(store);
  try {
    ...
    save(store);
    res.redirect("/cong-no?success=" + encodeURIComponent(`Đã tải file MISA (${parsed.order.length} công trình). Chọn công trình MISA tương ứng cho từng gian bên dưới.`));
  } catch (e) {
    res.redirect("/cong-no?error=" + encodeURIComponent(e.message));
  }
});

router.post("/cong-no/misa-mapping", (req, res) => {
  const store = load();
  ensureMisaShape(store);
  try {
    ...
    save(store);
    res.redirect("/cong-no?success=" + encodeURIComponent("Đã lưu ánh xạ Gian ↔ Công trình MISA."));
  } catch (e) {
    res.redirect("/cong-no?error=" + encodeURIComponent(e.message));
  }
});
